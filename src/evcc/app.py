...
def perform_api_cycle(
    *,
    ...
    result_payload = build_placeholder_result()
    client.set_input_text(
        config.result_helper_entity,
        dump_result_payload(result_payload),
    )
    logger.info("Wrote placeholder result to helper '%s'.", config.result_helper_entity)
...
def main() -> int:
    ...
    if client is None:
        ...
        try:
            perform_api_cycle(client=client, config=config, logger=logger)
        except HomeAssistantApiError as exc:
            logger.error("Home Assistant API cycle failed: %s", exc)
            try:
                client.set_input_text(
                    config.result_helper_entity,
                    dump_result_payload(build_error_result(str(exc))),
                )
                logger.info(
